package com.minegusta.mauswashere.location;
...
public class MLocation implements ConfigurationSerializable {
    private UUID id;
    String world;
    Double X;
    Double Y;
    ...
    public MLocation(UUID id, ConfigurationSection conf) {
        this.id = id;
    ...
    public static class Util {
        public static void save(MLocation location) {
        ...
        public static MLocation load(UUID id) {
            return DataManager.locations.get(id);
        }

        public static MLocation create(String world, double X, double Y, double Z, float yaw, float pitch) {
            MLocation trackedLocation = new MLocation();
        ...
        public static MLocation create(Location location) {
            return create(location.getWorld().getName(), location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        }

        public static Set<MLocation> loadAll() {
            return Sets.newHashSet(DataManager.locations.values());
        }

        public static MLocation get(final Location location) {
            try {
                return Iterators.find(loadAll().iterator(), new Predicate<MLocation>() {
                    @Override
                    public boolean apply(MLocation tracked) {
